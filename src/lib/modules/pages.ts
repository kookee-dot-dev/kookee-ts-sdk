...
export class PagesModule {
  ...
  async list(params?: PagesListParams, signal?: AbortSignal): Promise<PaginatedResponse<PageEntryListItem>> {
    return this.entries.list({ type: 'page', ...params }, signal) as unknown as Promise<
      PaginatedResponse<PageEntryListItem>
    >;
  }

  async getBySlug(slug: string, params?: PagesGetBySlugParams, signal?: AbortSignal): Promise<PageEntryDetail> {
    return this.entries.getBySlug(slug, { type: 'page', ...params }, signal) as unknown as Promise<PageEntryDetail>;
  }
